...
class SystemNotify(SystemNotifyMixin, NotifyMethodsMixin):
    ...
    @staticmethod
    def send(data_json: dict, user: User = None, event: int = None, room_name: str = None, title: str = '',  # noqa: C901
             **kwargs):

        if user is None and event is None:
            raise ValidationError('You must pass one of the "user" or "event" arguments')

        if not isinstance(data_json, dict):
            raise ValidationError('The "data_json" type is not dict')

        if user and not isinstance(user, User):
            raise ValidationError('The "user" argument must be a "User" instance')
        ...
        if template:
            ...
            if not isinstance(notify_json, dict):
                raise ValidationError('The template text type is not dict')
